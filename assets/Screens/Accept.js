...
console.log(myToken,'acept token')
  ...
  const fetchData = async () => {
    if(myToken)
    {
      ...
      const response = await fetch(
        `https://delivigo-api.herokuapp.com/api/v5/restaurant/orders?status=30&PageNo=${currentPage}`,
        {
          headers: {
  ...
  const fetchData2 = async (newPage) => {
    ...
    console.log(`https://delivigo-api.herokuapp.com/api/v5/restaurant/orders?status=30&PageNo=${newPage}`, 'check api')
    setisLoading(true)
    
    const response = await fetch(
      `https://delivigo-api.herokuapp.com/api/v5/restaurant/orders?status=30&PageNo=${newPage}`,
      {
        headers: {
          Authorization: `Bearer ${myToken}`, // SET HEADER IN TOKEN
        },
